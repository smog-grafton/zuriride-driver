  ...
  Widget build(BuildContext context) {
    return GetBuilder<ProfileController>(
      builder: (profileController) => switch (fieldModel.fieldType) {
        ...
        AdditionalFieldType.email ||
        AdditionalFieldType.textarea =>
          _TextInputField(
              fieldModel: fieldModel, profileController: profileController),
        AdditionalFieldType.phone => _PhoneInputField(
            fieldModel: fieldModel, profileController: profileController),
        AdditionalFieldType.date => _DateField(
            fieldModel: fieldModel, profileController: profileController),
        AdditionalFieldType.file => _FileField(
            fieldModel: fieldModel, profileController: profileController),
        AdditionalFieldType.checkbox => _CheckboxField(
            fieldModel: fieldModel, profileController: profileController),
        AdditionalFieldType.radio => _RadioField(
            fieldModel: fieldModel, profileController: profileController),
        AdditionalFieldType.select => _DropDownField(
            fieldModel: fieldModel, profileController: profileController),
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return _FieldColumn(
      ...
      child: TextFieldWidget(
        ...
        inputAction: TextInputAction.next,
        countryDialCode:
            profileController.additionalFieldPhoneCountryCodes[fieldModel.id],
        onCountryChanged: (code) => profileController
            .setAdditionalFieldPhoneCountryCode(fieldModel.id, code.dialCode!),
      ),
    );
  }
}
...
class _DateField extends StatelessWidget {
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    String? dateText =
        profileController.additionalFieldControllers[fieldModel.id]?.text;
      ...
      child: GestureDetector(
        ...
        child: AbsorbPointer(
          child: CustomTextField(
            ...
            controller:
                profileController.additionalFieldControllers[fieldModel.id],
            focusNode:
                profileController.additionalFieldFocusNodes[fieldModel.id],
            inputAction: TextInputAction.next,
            prefix: false,
            read: true,
          ),
...
class _FileField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return UpdateAdditionalFilesWidget(
      ...
      selectedFiles:
          profileController.additionalFieldFiles[fieldModel.id] ?? [],
      onFilesSelected: (List<dynamic> files) =>
          profileController.setAdditionalFieldFile(fieldModel.id, files),
    );
  }
}
...
class _CheckboxField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedOptions = (profileController
            .additionalFieldCheckValues[fieldModel.id] as List?) ??
        [];
          ...
          return _OptionRow(
            ...
            leading: _CheckBox(
              ...
              onChanged: (value) {
                ...
                profileController.setAdditionalFieldCheckValue(
                    fieldModel.id, updated);
              },
            ),
          );
        }).toList(),
        ...
        child: DropdownButtonHideUnderline(
          child: DropdownButton<String>(
            value: (selectedValue == null || selectedValue.isEmpty)
                ...
                : selectedValue,
            hint: Text(
                titleFormatedName(fieldModel.placeholder ?? fieldModel.title),
                style:
    ...
    return Column(
      ...
      children: [
        TextFieldTitleWidget(
            title: titleFormatedName(title), isRequired: isRequired),
