...
class UpdateAdditionalFilesWidget extends StatelessWidget {
  const UpdateAdditionalFilesWidget({
    super.key,
  ...
  final ValueChanged<List<dynamic>> onFilesSelected;
  ...
  Future<void> _pickFile(BuildContext context) async {
    if (selectedFiles.length >= (fieldModel.fileQuantity ?? 1)) {
      showCustomSnackBar(
          '${'max_file_limit_reached'.tr} ${fieldModel.fileQuantity}');
      return;
    }

    try {
      final List<String> allowedExtensions =
          fieldModel.allowedFileFormats ?? [];
      ...
      final pickedFile = await FileValidationHelper.validateAndPickFile(
        ...
        maxFileSizeInMB:
            Get.find<SplashController>().config?.maxImageUploadSize,
      );
      if (pickedFile != null) {
        final updated = List<dynamic>.from(selectedFiles)..add(pickedFile);
        onFilesSelected(updated
            .map(
                (e) => e is String ? e : (e is XFile ? e : XFile(e.toString())))
            .toList());
      }
    } catch (e) {
      assert(() {
          ...
          if (selectedFiles.isNotEmpty) ...[
            ...
            ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final String path =
                    file is String ? file : (file as XFile).path;
                ...
                return isImage
                    ? _ImagePreview(
                        ...
                        onRemove: () {
                          final updated = List<dynamic>.from(selectedFiles)
                            ..removeAt(index);
                          onFilesSelected(updated
                              .map((e) => e is String
                                  ? e
                                  : (e is XFile ? e : XFile(e.toString())))
                              .toList());
                        },
                      )
                    : _FileRow(
      ...
      child: Stack(
        children: [
          ClipRRect(
            ...
            child: file is String
                ? ImageWidget(
                    image:
                        '${Get.find<SplashController>().config?.imageBaseUrl?.additionalData}/$file',
                    width: previewWidth,
...
class _FileRow extends StatelessWidget {
  ...
  String get _fileName => file is String
      ? file.split('/').last
      : (file as XFile).path.split('/').last;
