  ...
  Widget build(BuildContext context) {
    return GetBuilder<ProfileController>(builder: (profileController) {
      return Column(children: [
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) {
              return _ImageAndDocumentView(
                imageBaseUrl: Get.find<SplashController>()
                        .config
                        ?.imageBaseUrl
                        ?.documents ??
                    '',
        ...
        if ((profileController.profileInfo?.additionalData ?? [])
            .isNotEmpty) ...[
          Container(
            padding:
            ...
            child: ListView.separated(
              ...
              itemCount:
                  (profileController.profileInfo?.additionalData ?? []).length,
              separatorBuilder: (_, __) =>
                  const SizedBox(height: Dimensions.paddingSizeSmall),
              itemBuilder: (context, index) {
                if (profileController
                        .profileInfo?.additionalData?[index].type ==
                    'file') {
                  return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                            titleFormatedName(profileController.profileInfo
                                    ?.additionalData?[index].title ??
                                ''),
                            style: textBold),
                        ...
                        ListView.separated(
                          ...
                          padding: EdgeInsets.zero,
                          physics: NeverScrollableScrollPhysics(),
                          itemCount: (profileController.profileInfo
                                      ?.additionalData?[index].value ??
                                  [])
                              .length,
                          itemBuilder: (context, idx) {
                            bool isImage = AppConstants.allowedImageExtensions
                            ...
                            return _ImageAndDocumentView(
                              imageBaseUrl: Get.find<SplashController>()
                                      ...
                                      ?.additionalData ??
                                  '',
                              imageUrl: profileController.profileInfo
                                      ?.additionalData?[index].value?[idx] ??
                                  '',
                  ...
                  String data = '';
                  if (['checkbox', 'radio', 'select'].contains(profileController
                      .profileInfo?.additionalData?[index].type)) {
                    data = titleFormatedName((profileController
                                .profileInfo?.additionalData?[index].value ??
                            [])
                        .join(", _"));
                  } else {
                  ...
                  return ProfileItemWidget(
                    title: titleFormatedName(profileController
                            .profileInfo?.additionalData?[index].title ??
                        ''),
                    value: data,
                    isLastIndex:
                        (profileController.profileInfo?.additionalData ?? [])
                                .length ==
                            (index + 1),
                  );
                }
        ...
        GestureDetector(
          ...
          child: Text(
              Get.find<ProfileController>().profileInfo?.loggedInVia != 'otp'
                  ? 'change_password'.tr
                  : 'set_password'.tr,
  ...
  Widget build(BuildContext context) {
    return isDocument
        ? InkWell(
            onTap: () async {
              Get.find<HelpAndSupportController>().downloadFile(
                  url: '$imageBaseUrl/$imageUrl}', fileName: imageUrl);
            },
            child: Container(
              ...
              child: Row(
                  ...
                  children: [
                    Row(children: [
                      Image.asset(ProfileHelper.checkImageExtensions(imageUrl),
                          height: 25, width: 25),
                      const SizedBox(width: Dimensions.paddingSizeSmall),
                      Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
        ...
        : ClipRRect(
            ...
            child: ImageWidget(
                image: '$imageBaseUrl/$imageUrl', height: Get.height * 0.1),
          );
  }
}
