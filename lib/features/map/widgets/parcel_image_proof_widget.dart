    ...
    return GetBuilder<OtpTimeCountController>(builder: (otpTimerController) {
      return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(
              froPickupImage
                  ? 'uploaded_picked_image'.tr
                  : 'upload_delivery_image'.tr,
              style: textSemiBold),
          ...
          SizedBox(
            ...
            child: ListView.separated(
              itemCount: froPickupImage
                  ? otpTimerController.parcelReceivingImage.length >= 5
                      ? 5
                      : otpTimerController.parcelReceivingImage.length + 1
                  : otpTimerController.parcelDeliveryImage.length >= 5
                      ? 5
                      : otpTimerController.parcelDeliveryImage.length + 1,
              scrollDirection: Axis.horizontal,
              itemBuilder: (ctx, index) {
                return index ==
                        (froPickupImage
                            ? otpTimerController.parcelReceivingImage.length
                            : otpTimerController.parcelDeliveryImage.length)
                    ? InkWell(
                        onTap: () {
                          Get.bottomSheet(
                              backgroundColor: Theme.of(context).cardColor,
                              ParcelProofBottomSheetWidget(
                                  froPickupImage: froPickupImage));
                        },
                        child: Container(
                          height: 60,
                          width: 60,
                    ...
                    : Stack(children: [
                        Container(
                          ...
                          child: ClipRRect(
                            ...
                            child: Image.file(
                              File(froPickupImage
                                  ? otpTimerController
                                      .parcelReceivingImage[index].path
                                  : otpTimerController
                                      .parcelDeliveryImage[index].path),
                              fit: BoxFit.cover,
                            ),
                          ),
                        ),
                        Positioned(
                          ...
                          child: InkWell(
                            onTap: () {
                              otpTimerController.removeImage(
                                  index, froPickupImage);
                            },
                            child: Container(
                              decoration: BoxDecoration(
