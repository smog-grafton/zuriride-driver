...
class AuthController extends GetxController
  ...
  Map<String, TextEditingController> get additionalFieldControllers =>
      _additionalFieldControllers;

  Map<String, FocusNode> _additionalFieldFocusNodes = {};
  Map<String, FocusNode> get additionalFieldFocusNodes =>
  ...
  Map<String, dynamic> get additionalFieldCheckValues =>
      _additionalFieldCheckValues;
  Map<String, String> _additionalFieldPhoneCountryCodes = {};
  Map<String, String> get additionalFieldPhoneCountryCodes =>
    ...
    if (response!.statusCode == 200) {
      if (configModel?.verification ?? false) {
        ...
        showCustomSnackBar('registration_completed_successfully'.tr,
            isError: false);
        login(code, signUpBody.phone?.replaceAll(code, '') ?? '',
            signUpBody.password ?? '');
      }
      Get.find<ProfileController>().updateFirstTimeShowBottomSheet(true);
    } else if (response.statusCode == 407) {
      Get.bottomSheet(ManualAuthWaringBottomSheetWidget(
          phoneNumber: signUpBody.phone?.replaceAll(code, '') ?? '',
  ...
  Future<void> registerWithOtp(SignUpBody signUpBody,
    ...
    if (response!.statusCode == 200) {
      ...
      updateToken().then((value) {
        Get.find<OutOfZoneController>().getZoneList();
        String countryCode =
            CountryCodeHelper.getCountryCode(signUpBody.phone) ?? '';
        _navigateLogin(
            countryCode,
            signUpBody.phone?.replaceAll(countryCode, '') ?? '',
            signUpBody.password ?? '');
      });
    } else {
      ApiChecker.checkApi(response);
        ...
        showDialog(
            ...
            builder: (_) => ApproveDialogWidget(
                ...
                onYesPressed: () {
                  ...
                  login(countryCode, phoneNumber.replaceAll(countryCode, ''),
                      password);
                }),
  ...
  void loadAdditionalFields(bool fromOtp) {
    ...
    if (fromOtp) {
      for (final field
          in Get.find<SplashController>().config?.additionalFieldList ?? []) {
        if (field.isRequired) {
      ...
      _additionalFields =
          Get.find<SplashController>().config?.additionalFieldList ?? [];
    }
  ...
  void _initAdditionalFieldControllers() {
    ...
    for (var field in _additionalFields) {
      if (field.fieldType == AdditionalFieldType.checkbox) {
      ...
      } else if (field.fieldType == AdditionalFieldType.radio ||
          field.fieldType == AdditionalFieldType.select) {
        ...
        if (field.fieldType == AdditionalFieldType.phone) {
          _additionalFieldPhoneCountryCodes[field.id] =
  ...
  Future<Map<String, dynamic>> getAdditionalFieldValues() async {
    ...
    for (var field in _additionalFields) {
      if (field.fieldType == AdditionalFieldType.checkbox ||
          field.fieldType == AdditionalFieldType.radio ||
          field.fieldType == AdditionalFieldType.select) {
        additionalData[field.title] = _additionalFieldCheckValues[field.id] ??
            (field.options?.isNotEmpty == true ? [] : false);
      } else if (field.fieldType == AdditionalFieldType.file) {
        _additionalFieldFiles[field.id]?.forEach((file) {
          additionalFiles
              .add(MultipartBody('additional_data[${field.title}][]', file));
        });
      } else {
  ...
  String? validateAdditionalFields() {
    for (var field in _additionalFields) {
      ...
      if (field.fieldType == AdditionalFieldType.checkbox) {
        ...
        if (_additionalFieldFiles[field.id] == null ||
            _additionalFieldFiles[field.id]!.isEmpty) {
          return field.title;
        }
      } else {
  ...
  Future<void> goToSettings() async {
    ...
    _bioList = await bioAuth.getAvailableBiometrics();
    if (_bioList.isEmpty &&
        (_settingsCompleter == null || _settingsCompleter!.isCompleted)) {
      try {
    ...
    authServiceInterface.pinVerify(password).then((response) async {
      if (response.statusCode == 200 && response.body != null) {
        ...
        authServiceInterface
            .setBiometricEnabled(isActive && _bioList.isNotEmpty);
        try {
          authServiceInterface.addOrUpdateOnLocalDatabase(
              AppConstants.biometricPin, password);
        } catch (error) {
  ...
  Future<void> updateBiometricPin(String password) async {
    await authServiceInterface.addOrUpdateOnLocalDatabase(
        AppConstants.biometricPin, password);
  }

  Future<String> getBiometricPin() async {
    return await authServiceInterface
        .getDataFromLocalDatabase(AppConstants.biometricPin);
  }
  ...
  void loginWithBiometric() async {
    ...
    if (isAbleToLogin &&
        await authenticateWithBiometric('please_authenticate_to_login'.tr)) {
      login(
