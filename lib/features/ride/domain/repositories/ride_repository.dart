...
class RideRepository implements RideRepositoryInterface {
  ...
  Future<Response> bidding(String tripId, String amount) async {
    return await apiClient.postData(
        AppConstants.bidding, {"trip_request_id": tripId, "bid_fare": amount});
  }
  ...
  Future<Response> tripAcceptOrReject(String tripId, String action) async {
    return await apiClient.postData(AppConstants.tripAcceptOrReject,
        {"trip_request_id": tripId, "action": action});
  }

  @override
  Future<Response> ignoreMessage(String tripId) async {
    return await apiClient
        .postData(AppConstants.tripAcceptOrReject, {"trip_request_id": tripId});
  }

  @override
  Future<Response> matchOtp(
      String tripId, String otp, List<MultipartBody> proofImages) async {
    return await apiClient.postMultipartData(
      AppConstants.matchOtp,
      {"trip_request_id": tripId, "otp": otp},
      proofImages,
      null,
      [],
    );
  ...
  Future<Response> startForPickup(String tripId) async {
    return await apiClient
        .putData("${AppConstants.outForPickupUri}$tripId", {});
  }
  ...
  Future<Response> tripStatusUpdate(
      ...
      String dateTime,
      List<MultipartBody> proofImages) async {
    return await apiClient.postMultipartData(
  ...
  Future<Response> getPendingRideRequestList(int offset,
      {int limit = 10}) async {
    return await apiClient
        .getData('${AppConstants.rideRequestList}?limit=$limit&offset=$offset');
  }
  ...
  Future<Response> getFinalFare(String tripId) async {
    return await apiClient
        .getData('${AppConstants.finalFare}?trip_request_id=$tripId');
  }
